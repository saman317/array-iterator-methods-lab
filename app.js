...
veryOldInventors = inventors.filter(inventor=>{
  if(inventor.year > 1499 && inventor.year <= 1599 ) return true
...
firstLast = people.map((peeps)=>{
  return peeps.split(', ').reverse().join(' ')
});
// Check your work:
...
isEveryone19OrOlder = devs.every((nineteen)=>{
  if(nineteen.year >= 2007)return true
})
...
idx = comments.findIndex((index)=>{
  if(index.id === 123523)return true
})
